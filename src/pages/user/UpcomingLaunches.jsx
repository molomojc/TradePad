  ...
  return (
    <PageTransition className="max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        ...
        <div className="flex items-center bg-surface-variant border border-outline-variant rounded-xl px-3 py-2 gap-2 focus-within:border-primary/50 transition-all flex-1 md:flex-none md:w-64">
      ...
      {loading ? (
        ...
        <div className="glass-card p-6 sm:p-10 rounded-[2rem] border-outline-variant hover:border-primary/20 transition-all duration-300 shadow-xl text-center relative overflow-hidden group">
          ...
          <span className="material-symbols-outlined text-5xl text-primary mb-4 relative z-10">workspace_premium</span>
          <h3 className="text-2xl text-on-surface font-bold mb-2 relative z-10">Premium launches are locked</h3>
          <p className="text-on-surface-variant max-w-2xl mx-auto mb-6 relative z-10">
          ...
          <Link to="/dashboard/user/previous" className="inline-flex bg-surface hover:bg-surface-variant border border-outline-variant text-on-surface px-6 py-3 rounded-xl font-mono text-sm transition-colors mt-2 relative z-10">
            Browse Past Coins
          </Link>
        </div>
      ) : (
        <>
          {hiddenLaunch && (
            <div className="glass-card p-6 sm:p-8 rounded-[2rem] border border-neon-red/20 hover:border-neon-red/40 mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-6 shadow-lg shadow-neon-red/5 hover:shadow-neon-red/10 transition-all duration-500 relative overflow-hidden group hover:-translate-y-1">
              ...
              <div className="grid grid-cols-2 gap-4 min-w-[280px] relative z-10">
                <div className="bg-surface-variant p-4 rounded-2xl border border-outline-variant">
                ...
                <div className="bg-surface-variant p-4 rounded-2xl border border-outline-variant">
                  ...
                  <p className="text-on-surface text-lg font-bold">{hiddenLaunch.joined_count}</p>
                </div>
              </div>
            </div>
          )}
          ...
          <motion.div variants={containerVariants} initial="hidden" animate="show" className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            {filtered.map((launch) => (
              <motion.div key={launch.id} variants={itemVariants} className="glass-card p-6 rounded-2xl border-outline-variant hover:border-primary/30 transition-all duration-300 hover:-translate-y-1 shadow-lg hover:shadow-primary/10 flex flex-col justify-between relative group overflow-hidden">
                <button
                  onClick={(event) => toggleWatchlist(event, launch.id)}
                  className="absolute top-6 right-6 z-10 w-8 h-8 rounded-full bg-surface/50 backdrop-blur-md flex items-center justify-center border border-outline-variant hover:bg-surface-variant transition-colors"
                >
                  <span className={`material-symbols-outlined text-[18px] transition-colors ${watchlist.has(launch.id) ? 'text-primary fill-current' : 'text-on-surface-variant'}`}>
                    star
                  </span>
                </button>
                ...
                <div>
                  <div className="flex items-center gap-4 mb-4">
                    {launch.logo_url && !launch.is_teaser ? (
                      <img src={launch.logo_url} alt={launch.name} className="w-12 h-12 rounded-xl object-cover shadow-lg border border-outline-variant" />
                    ) : (
                      <div className="w-12 h-12 rounded-xl bg-surface-variant flex items-center justify-center text-xl shadow-lg border border-outline-variant text-on-surface font-bold">
                  ...
                  <div className="flex flex-wrap items-center gap-2 mb-6">
                    ...
                    <span className="bg-surface-variant text-on-surface-variant border border-outline-variant px-2 py-0.5 rounded font-mono text-[10px]">{launch.chain}</span>
                ...
                <div className="border-t border-outline-variant pt-4 mt-auto">
                  ...
                  <div className="grid grid-cols-3 gap-2 items-center mb-4">
                    <div>
                      ...
                      <p className="text-[11px] text-on-surface font-medium">{launch.launch_at ? new Date(launch.launch_at).toLocaleDateString() : 'TBA'}</p>
                    </div>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-[11px] text-on-surface font-bold">{formatCountdown(launch.launch_at)}</p>
                    </div>
                  </div>

                  <Link to={`/dashboard/user/launch/${btoa(launch.id)}`} className="w-full block text-center bg-surface hover:bg-surface-variant text-on-surface py-3 rounded-xl font-mono text-xs transition-colors border border-outline-variant relative overflow-hidden group-hover:border-primary/30">
          ...
          {filtered.length === 0 && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center py-24 glass-card rounded-3xl border border-outline-variant relative overflow-hidden group">
              ...
              <span className="material-symbols-outlined text-6xl mb-4 text-on-surface-variant/50 relative z-10 group-hover:scale-110 transition-transform duration-500">search_off</span>
