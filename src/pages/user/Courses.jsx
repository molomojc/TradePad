...
export default function Courses() {
  ...
  return (
    <PageTransition className="max-w-7xl mx-auto space-y-6 pb-12">
      {/* Hero Header */}
      <div className="relative rounded-3xl border border-outline-variant bg-surface p-8 md:p-12 overflow-hidden shadow-2xl">
          ...
          {courses.map((course) => {
            ...
            return (
              <motion.div
                ...
                className="glass-card flex flex-col justify-between overflow-hidden rounded-2xl border-outline-variant hover:border-primary/20 transition-all duration-300 hover:-translate-y-1 shadow-lg hover:shadow-primary/5 group"
              >
                {/* Card Thumbnail */}
                <div className="relative aspect-video w-full overflow-hidden bg-black/40 border-b border-outline-variant">
                  {course.thumbnail_url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-white/5 to-transparent">
                      <span className="material-symbols-outlined text-4xl text-on-surface/25">school</span>
                    </div>
                  )}
                  ...
                  <div className="absolute top-4 left-4 flex flex-wrap gap-2 z-10">
                    {isPremiumCourse ? (
                      ...
                      <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded border border-outline-variant bg-surface-variant text-on-surface text-[10px] font-mono font-bold uppercase tracking-wider">
                        Free
                      </span>
                    )}
                  ...
                  {isLocked && (
                    <div className="absolute inset-0 bg-black/70 backdrop-blur-[3px] flex flex-col items-center justify-center text-center p-4">
                      ...
                      <p className="text-on-surface text-xs font-mono font-bold uppercase tracking-wider">Premium Access Required</p>
                    </div>
                  )}
                </div>
                ...
                <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                  <div className="space-y-2">
                    ...
                    <h3 className="font-display text-lg text-on-surface font-bold group-hover:text-primary transition-colors line-clamp-1">{course.title}</h3>
                  ...
                  {isLocked ? (
                    ...
                    <Link
                      ...
                      className="w-full py-2.5 rounded-xl bg-surface-variant border border-outline-variant hover:bg-surface-variant text-on-surface text-center font-mono font-bold text-xs transition-all flex items-center justify-center gap-1 group-hover:border-primary/30 group-hover:bg-primary/5"
                    >
