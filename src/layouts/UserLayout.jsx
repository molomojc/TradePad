    ...
    <div className="relative flex min-h-screen overflow-x-hidden bg-background text-on-background">
      ...
      <main className="relative flex min-h-screen min-w-0 flex-1 flex-col md:ml-[260px]">
        <header className="sticky top-0 z-30 flex h-20 items-center justify-between gap-4 border-b border-outline-variant bg-background/90 px-4 backdrop-blur-xl sm:px-6 lg:px-8">
          ...
          <div data-tour="header-actions" className="flex shrink-0 items-center gap-2">
            <button type="button" onClick={() => { toggleTheme(); toast.success(`Switched to ${isLight ? 'dark' : 'light'} mode`); }} className="header-icon-button" aria-label={isLight ? 'Switch to dark mode' : 'Switch to light mode'}><span className="material-symbols-outlined text-[19px]">{isLight ? 'dark_mode' : 'light_mode'}</span></button>
            <button type="button" onClick={() => window.dispatchEvent(new CustomEvent('launch-tradepad-tour'))} className="header-icon-button" aria-label="Restart tour guide" title="Restart tour guide"><span className="material-symbols-outlined text-[19px]">help_outline</span></button>
            ...
            <button type="button" className="flex items-center gap-3 rounded-lg p-1.5 text-left transition-colors hover:bg-surface-variant/50">
              {profile?.avatar_url ? (
                <img 
